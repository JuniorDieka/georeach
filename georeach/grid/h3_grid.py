...
def generate_and_load_grid(config: Config) -> None:
    ...
    admin_gdf = gpd.read_postgis(
        "SELECT geometry FROM admin_boundaries", engine, geom_col="geometry"
    )
    ...
    h3_indexes = h3.polyfill_geojson(study_area_4326.__geo_interface__, config.h3.resolution)
    ...
    with engine.connect() as conn:
        conn.execute(
            text("CREATE INDEX IF NOT EXISTS h3_grid_geom_idx ON h3_grid USING GIST (geometry)")
        )
        conn.commit()
